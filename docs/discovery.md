...
- **Rezerve Stok & Net Kapasite Hesabı:** `stock.quant`'tan `quantity` (On-Hand) yanı sıra `reserved_quantity` alanı da okunur. Tablodaki "Net Stock" sütunu ve tüm üretilebilir cihaz (`max_devices`) / ihtiyaç (`req_20_val`, dinamik hedefler) hesaplamalarında `avail_qty = stock_qty - reserved_qty` baz alınarak rezerve stoklar tamamen düşülmüş net stok gösterilir. Hücre tooltip'inde brüt eldeki stok ve rezerve miktarları detaylı görünür.
