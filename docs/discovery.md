...
- **Rezerve Stok & Net Kapasite Hesabı:** `stock.quant`'tan `quantity` yanı sıra `reserved_quantity` alanı da okunur. Üretilebilir cihaz (`max_devices`) ve ihtiyaç (`req_20_val`, dinamik hedefler) hesaplamalarında `avail_qty = stock_qty - reserved_qty` baz alınarak rezerve stoklar tamamen düşülür.
